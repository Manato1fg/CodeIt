...
public class DFT {
    ...
    public static FourierData[] dft(double[] data) {
        int N = data.length;
        ...
        for (int k = 0; k < N; k++) {
            ...
            for (int n = 0; n < N; n++) {
                ComplexNumber cn2 = new ComplexNumber(Math.cos(2 * n * k * Math.PI / N), -Math.sin(2 * n * k * Math.PI / N));
                cn2.multiply(data[n]);
                cn.add(cn2);
            }
        ...
        Collections.sort(_data, (a, b) -> { return (int) (b.cn.amp - a.cn.amp); });
